...
<form class="admin-form" action="{{route('admin.store_model', ['table'=>$model::nameTable()])}}" enctype="multipart/form-data" method="POST">
    ...
    <div class="form-body">
        ...
        <div class="form-group">
            ...
            <input type="number" class="form-control @if($errors->has('number_breaks')) {{'is-invalid'}} @endif" name="number_breaks" value="{{ old('number_breaks') }}" placeholder="Заполните поле '{{$model::get_ru_field('number_breaks')}}'">
        ...
        <div class="form-group">
            ...
            <input list="defaultNumbers" step='5' type="number" class="form-control input_minutes @if($errors->has('duration_minutes')) {{'is-invalid'}} @endif" name="duration_minutes" value="{{ old('duration_minutes') }}" placeholder="Заполните поле '{{$model::get_ru_field('duration_minutes')}}'">
        ...
        <div class="form-group">
            ...
            <input type="time" class="form-control @if($errors->has('time_start')) {{'is-invalid'}} @endif" name="time_start" value="{{ old('time_start') }}">
            @if($errors->has('time_start'))
                <div class="error">{{$errors->first('time_start')}}</div>
            @endif
        </div>
        <div class="form-group">
            ...
            <input type="time" class="form-control @if($errors->has('time_end')) {{'is-invalid'}} @endif" name="time_end" value="{{ old('time_end') }}">
